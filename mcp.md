...
- **library** — import / remove / reorder / sort / shuffle / select / **keep_best** (`confirm: true` required — same as Library **⋯** → **Keep Best Shots…**; extras move to Outtakes; videos are never Keep Best targets; undo with **document** `undo`) / **move_to_outtakes** / **move_to_takes** (same as the context menu; all given paths or the current selection). Same ingest as **Add / +**, Finder drop (files or a folder), Photos.app drag, and paste. Finder / folder imports stay **linked**; **Photos.app** media is **copied** into Application Support **Imports** (no stable links into the Photos library). After a UI import, MemoryString may also prompt Keep Best when similar photo groups appear.
