...
- **style** — Looks, Energy, Photo Size, stage, customize knobs (including Anamorphic Streaks and Refract Bubbles knobs and presets), **atmosphere** (`none` / `bubbles` / `leaves`) and **decals** (`none` / `travel` / `vacation` / `party` / `florals` / `wedding` / `pets` / `sports`) as single-choice menus, and **clip_lens** (Studio per-slide Lens Effect hand picks; consecutive Refract Bubbles pins share one field). Look apply and Shuffle Transitions re-deal automatic slides only — hand picks stay until cleared; pool checkboxes and How often govern automatic slides only; untick then re-tick restores automatic picks
...
- **export_movie** — write an MP4 to a path you pass (no save panel). Optional `quality` (`low` / `medium` / `high` / `maximum` → Email / Share / Screen / Master; **Share** / `medium` default), `quality_level` (0…1), `resolution` (`1080p` / `4k`; not available for Cinema 21:9), `fps`, `format`, `social_safe`
