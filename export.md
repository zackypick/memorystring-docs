...
![Export Movie: Save As, Format, Resolution, Social Safe, Quality (Compact–Best), Frame Rate, Projected size](../.gitbook/assets/export-dialog.png)
...
- **Resolution** — **1080p** (default) or **4K**. 4K takes much longer — around three times the length of the show. **Cinema (21:9)** does not offer 4K — that format stays at 1080p because of the aspect, not the Mac. Rarely, when other apps leave too little free memory, 4K declines to start and asks you to close some apps or use 1080p — so you are not left waiting. If memory tightens mid-export, the encode slows rather than failing.
- **Social Safe**
- **Quality** — a slider with four stops: **Compact**, **Share**, **High**, **Best**. **Share** is the default. The stop sets the encoder bit rate — smallest file at Compact, highest quality at Best. Same control in Essential and Studio; Studio also shows the target rate in Mbps ([Essential and Studio](workspace/essential-studio.md)).
